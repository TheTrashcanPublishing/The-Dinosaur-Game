...
void tog::DisplayStartScreen(void) {
    togi::DrawGameScreen();
    togi::DrawTitleInfo();
//    PlayThemeMusic;
...
//    DisplayStandardGameBackground;
    ...
    int PlayerFigure = 2; /* There are 3 player figures according to if the player is standing still (PlayerFigure = 2),
                              moving to the left (PlayerFigure = 1), or moving to the right (PlayerFigure = 3). */
        ...
        while ((DinosaurAlive) && (PlayerAlive)) {
            if (NoJoystickMovement) || (JoystickUp) || (JoystickDown)) {
                ...
                if (DinosaurFigure <= 4) {
                    ...
                    else if (Level < (MaxLevel – 2)) Wait(Level + 2);  // <<== (Wait)
                    else Wait(MaxLevel);    }  // <<== (Wait)
            ...
            if ((JoystickRight) || (JoystickDiagUpRight) || (JoystickDiagDownRight)) {
                ...
                bool DinoHit = FALSE;
                bool SpearFigure = FALSE; // There are 2 spear figures, one for shooting up at 45 deg angle and one for going straight down
